...
VelocityControl::VelocityControl(int encoderPinA, int encoderPinB, int motorPWMPin, int motorIn1Pin, int motorIn2Pin, float _kp)
    : encoder(encoderPinA, encoderPinB), motor(motorPWMPin, motorIn1Pin, motorIn2Pin) 
{
...
void VelocityControl::control()
{
    ...
    filteredVelocity = actualVelocity;

    // Serial.println(actualVelocity);

    float error = setpoint - rawVelocity;
...
void VelocityControl::printError()
{
    Serial.print("Error: ");
    Serial.println(setpoint - filteredVelocity);
}
